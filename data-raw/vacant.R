...
ct_tracts <- cwi::xwalk |>
  dplyr::distinct(town, cog_fips, tract_cog) |>
  dplyr::filter(town %in% c("Stamford", "New Haven")) |>
  tidyr::separate_wider_position(cog_fips, widths = c(state = 2, county = 3)) |>
  dplyr::rename(city = town, tract = tract_cog)

geos <- dplyr::distinct(ct_tracts, state, city, county) |>
  dplyr::add_row(state = "24", city = "Baltimore", county = "510")

tracts <- geos |>
  dplyr::mutate(data = purrr::pmap(geos, function(state, city, county) {
    tigris::tracts(state = state, county = county, cb = TRUE, year = 2022) |>
      janitor::clean_names()
  })) |>
  tidyr::unnest(data) |>
  dplyr::select(state, city, county, geoid, aland) |>
  dplyr::filter(state != "09" | geoid %in% ct_tracts$tract)

housing <- purrr::pmap(geos, function(state, city, county) {
