...
county_sf <- tigris::counties(state = "24") |>
    ...
    dplyr::filter(
        county %in%
            c("Baltimore city", "Baltimore County", "Anne Arundel County")
    )
...
art <- dplyr::select(
    ...
    site,
    visibility,
    access = indoor_outdoor_accessible,
    loc_tmp
)
...
art <- dplyr::mutate(
    art,
    dplyr::across(dplyr::where(is.character), \(x) dplyr::na_if(x, "n/a"))
)
...
art$geometry <- stringr::str_replace_all(
    art$geometry,
    "\\((.+), (.+)\\)",
    "POINT(\\2 \\1)"
)
